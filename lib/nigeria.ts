 ...
 *
 * Generated, and checked on generation against the totals fixed by law: 36
 * states plus the Federal Capital Territory, and 774 LGAs between them. States
 * are in alphabetical order and so are the LGAs within each one.
 *
 * Only asked of participants who live in Nigeria — an LGA is a Nigerian
 * administrative division and means nothing elsewhere.
 */

export interface NigerianState {
  readonly state: string;
  readonly lgas: readonly string[];
}
...
export const STATE_NAMES = NIGERIA_STATES.map((s) => s.state) as unknown as readonly [string, ...string[]];

const LGAS_BY_STATE = new Map<string, readonly string[]>(NIGERIA_STATES.map((s) => [s.state, s.lgas]));

/** The LGAs of one state, or an empty list when the state is not recognised. */
export function lgasOf(state: string): readonly string[] {
  return LGAS_BY_STATE.get(state) ?? [];
}

/** True when `state` is a Nigerian state and `lga` is one of its LGAs. */
export function isLgaOf(state: string, lga: string): boolean {
  return lgasOf(state).includes(lga);
}

/** True when the name is one of the 36 states or the FCT. */
export function isState(state: string): boolean {
  return LGAS_BY_STATE.has(state);
}
